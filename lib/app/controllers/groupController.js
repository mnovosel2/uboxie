...
module.exports = {
    ...
    addNewGroup: function(req, res) {
        var groupName = req.body.groupName,
            token = req.body.token || req.session.deezer.token;
        if (groupName && token) {
            async.waterfall([
                function(callback) {
                    request('http://localhost/api/1/user/current/' + token, function(err, res, body) {
                        if (err) {
                            callback(err);
                        } else {
                ...
                /*function(groupData, callback){
                    ...
                    models.active.findOne({ id: groupData._id }, function(err, kifla){

                        console.log('activegrouperror: ' + err);

                        console.log('ActveGroup: ' + kifla);
                ...
                function(groupData, callback){
                    new models.active({ id: groupData._id }).save(function(err, savedGroup){
                        if(err){
                            callback(err);
                        } else {
                            //do nothing, group is saved!
                            res.send({
                                status: true,
                                group: groupData
                            });
