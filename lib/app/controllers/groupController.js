...
module.exports = {
    ...
    addNewGroup: function(req, res) {
        if (req.body.groupName) {
            ...
            //TODO: dodat uzimanje ownera
            
            console.log(req.session.deezer.token);
            ...
            request('http://localhost/api/1/user/current/' + req.session.deezer.token, function(err, res, body){
                ...
                models.user.findOne({
                    ...
                    if(err){
                        ...
                        if(wantedUser != null){
                            console.log('wantedUserGroupCreation:');
                            console.log(wantedUser);
                            new models.music_group({
                                name: req.body.groupName,
                                /*sending object dosen't work*/
                                owner: {
                                    id: wantedUser.id,
                                    name: wantedUser.name,
                                    lastname: wantedUser.lastname,
                                    firstname: wantedUser.firstname,
                                    birthday: wantedUser.birthday,
                                    inscription_date: wantedUser.inscription_date,
                                    gender: wantedUser.gender,
                                    link: wantedUser.link,
                                    picture: wantedUser.picture,
                                    country: wantedUser.country,
                                    lang: wantedUser.lang,
                                    tracklist: wantedUser.tracklist,
                                    type: wantedUser.type,
                                    status: wantedUser.status
                                }
