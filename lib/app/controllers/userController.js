    ...
    loginCallback: function(req, res,next) {
        ...
        if (!code) {
            ...
            deezer.createSession(appId, appSecret, code, function(err, result) {
                ...
                asnyc.waterfall([
                    function(callback){
                        request('http://localhost/api/1/user/current/' + result.accessToken, function(err, res, body){
                            if(err){
                                callback(err);
                            }
                            else{
                    ...
                    function(userData, callback){
                        new models.user(userData).save(function(err, user){
                            if(err)
                                callback(err);
                            else{
                                callback('User saved!');
                ...
                request('http://localhost/api/1/user/current/' + result.accessToken, function(err, res, body){
                    ...
                    models.user.findOne({
                        id: parsed.id
                    }, function(error, wantedUser){
                        if(error)
                            console.log(error);
                        else{
                            if(wantedUser == null){
                                new models.user(parsed).save(function(err, user){
